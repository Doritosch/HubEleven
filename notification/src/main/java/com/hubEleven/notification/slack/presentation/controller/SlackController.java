...
public class SlackController {
    ...
    @Operation(
            summary = "메시지 발송 API",
            description = "AI를 통해 메시지를 생성하고 Slack으로 발송한다."
    )
    @PostMapping
    ...
    @Operation(
            summary = "메시지 수정 API",
            description = "기존 Slack 메시지를 수정합니다."
    )
    ...
    @Operation(
            ...
            description = "특정 Slack 메시지의 상세 정보를 조회한다."
    )
    ...
    @Operation(
            summary = "메시지 목록/검색 API",
            description = "조건에 따라 Slack 메시지 목록을 조회한다."
    )
    @GetMapping
    public ResponseEntity<ApiResponse<CommonPageResponse<SlackMessageResponse>>> searchMessages(
        ...
        // TODO: 권한 체크 - MASTER만 가능
        var page = slackMessageAppService.searchMessages(
                status,
                channel,
                dateFrom,
                dateTo,
